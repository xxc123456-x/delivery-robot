...
```
   ...
   └────┬────┘                  └────┬────┘                  └────┬────┘
        │                            │                            │
   ├─ /scan (激光雷达)               │                            │
   ├─ /camera/rgb (RGB图)           ├─ simple_go_hw.py           │
   ├─ /camera/depth (深度图)       │   ├─ A* 全局规划            │
   ├─ ai_voice.py (语音→文本)      │   ├─ VFH+ 局部避障         │
   ├─ ai_vision.py (图像→门牌号)   │   ├─ 配送队列调度           │
   ├─ depth_camera.py (3D障碍)     │   └─ 状态监控               │
   └─ /odom_fused (融合里程计)      │                            │
                                    │                         /cmd_vel_cmd
                                    │                            │
                               ┌────▼────┐                  ┌────▼────┐
...
- **定位**：订阅 `/odom_fused`（IMU+轮式融合里程计）， `/odom` 兜底
...
- **指令解析**：正则匹配提取房间号（"去302"、"送餐到103"）、坐标（"x2.5y1.5"）、别名（"回原点"）
...
1. 上电启动，等待各节点就绪（约 30 秒）
2. 手机打开 App 或网页，确认连接状态绿灯
3. **语音下单**："去 302 房间" → 机器人自动导航
4. **二维码识别**：行驶中自动扫描门上号码，确认到达
5. 配送完成，语音播报，自动返回待命区
